...
const menuGroups = [
  ...
  },
  {
    ...
    title: 'شؤون الطلاب',
    items: [
      { id: 'student-affairs', label: 'لوحة شؤون الطلاب الرئيسية', icon: Users, status: 'enhanced' },
      { id: 'student-enrollment', label: 'تسجيل الطلاب', icon: UserCheck, status: 'enhanced' },
      { id: 'student-records', label: 'ملفات الطلاب', icon: FileText, status: 'new' },
      { id: 'student-attendance', label: 'حضور وغياب الطلاب', icon: ClipboardList, status: 'enhanced' },
      { id: 'student-behavior', label: 'السلوك الطلابي', icon: MessageSquare, status: 'new' },
      { id: 'student-transfers', label: 'النقل والتحويل', icon: UserCheck, status: 'new' },
      { id: 'student-scholarships', label: 'المنح الدراسية', icon: GraduationCap, status: 'enhanced' },
      { id: 'student-counseling', label: 'الإرشاد الطلابي', icon: MessageSquare, status: 'new' },
      { id: 'student-activities', label: 'الأنشطة الطلابية', icon: Calendar, status: 'enhanced' }
    ]
  },
  {
    id: 'academic',
    ...
    items: [
      ...
      { id: 'programs', label: 'البرامج الأكاديمية', icon: GraduationCap, status: 'enhanced' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'quick-services', label: 'إدارة الخدمات السريعة', icon: Zap, status: 'enhanced' },
  ...
  },
  {
    ...
    title: 'إدارة المحتوى',
    items: [
      { id: 'hero-section', label: 'إدارة القسم الرئيسي', icon: Layout, status: 'new' },
  ...
  },
  {
    ...
    items: [
      { id: 'theme-customization', label: 'تخصيص المظهر', icon: Palette, status: 'new' },
      { id: 'layout-management', label: 'إدارة التخطيط', icon: Layout, status: 'new' }
    ]
  },
  {
    id: 'system',
...
export function ModernAdminSidebar({ currentSection, onSectionChange }: ModernAdminSidebarProps) {
  ...
  return (
    <Sidebar 
      className="sticky top-0 h-screen z-40 border-r border-border/40 bg-gradient-to-b from-admin-sidebar to-admin-bg/50 backdrop-blur-sm" 
      collapsible="icon"
      side="left"
    >
      {/* Header */}
      ...
      <SidebarContent className="px-2">
        {menuGroups.map((group) => (
          <SidebarGroup key={group.id} className="py-2">
            {!isCollapsed && (
              <SidebarGroupLabel className="text-xs font-semibold text-muted-foreground/80 px-3 mb-2">
                {group.title}
              </SidebarGroupLabel>
            )}
      ...
      <div className="p-4 border-t border-border/40 bg-gradient-to-r from-admin-header/5 to-admin-accent/5">
        {!isCollapsed && (
          <div className="text-center space-y-1">
            <div className="text-xs font-medium text-admin-header">
              نظام إدارة متطور
            </div>
            <div className="text-xs text-admin-accent">
              إصدار 3.0 - 2024
            </div>
          </div>
        )}
