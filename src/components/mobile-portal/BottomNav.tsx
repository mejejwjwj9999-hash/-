      ...
      <div className="flex items-center justify-around py-2 px-1">
        {mainTabs.map((tab) => {
          ...
          return (
            <Button
              ...
              className={`flex flex-col items-center justify-center p-2 min-h-[60px] flex-1 max-w-[80px] transition-all duration-200 ${
                isActive 
            ...
            >
              ...
              <span className={`text-xs font-medium text-center leading-tight ${
                isActive ? 'text-university-blue' : 'text-muted-foreground'
              }`}>
                {tab.label}
              </span>
              {isActive && (
                <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-university-blue rounded-full" />
              )}
            </Button>
          );
        })}
        ...
        {hasMoreItems && (
          <Button
            variant="ghost"
            onClick={onMoreClick}
            className="flex flex-col items-center justify-center p-2 min-h-[60px] flex-1 max-w-[80px] text-muted-foreground hover:text-university-blue hover:bg-university-blue/5 transition-all duration-200"
            aria-label="المزيد"
          >
            <div className="mb-1">
