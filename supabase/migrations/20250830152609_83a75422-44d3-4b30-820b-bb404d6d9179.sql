...
DROP POLICY IF EXISTS "Students can request new documents" ON public.documents;
CREATE POLICY "Students can request new documents" 
ON public.documents 
FOR INSERT 
WITH CHECK (
  student_id IN (
...
-- السماح للطلاب بتحديث طلباتهم
DROP POLICY IF EXISTS "Students can update their own service requests" ON public.service_requests;
CREATE POLICY "Students can update their own service requests" 
ON public.service_requests 
FOR UPDATE 
USING (
  student_id IN (
